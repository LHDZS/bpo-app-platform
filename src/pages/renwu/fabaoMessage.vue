...
<template>
  <div class="fabaoMessage">
    <div class="box" v-show="releaseType">
      <el-row>
          <el-form ref="releaseForm" :model="releaseForm" label-width="120px">
              <el-col :span="8">
                  <el-form-item label="任务ID：">
                      <el-input v-model="releaseForm.platformTaskNo" size="small" placeholder="请输入任务ID"></el-input>
                  </el-form-item>
              </el-col>
                <el-col :span="8">
                  <el-form-item label="任务需求方：">
                      <el-input v-model="releaseForm.companyName" size="small" placeholder="请输入任务需求方"></el-input>
                  </el-form-item>
              </el-col>
              <el-col :span="8">
                  <el-form-item label="任务状态：">
                      <el-select v-model="releaseForm.taskStatus" style="width:100%" placeholder="请选择任务状态" size="small">
                          <el-option
              ...
              <el-col :span="24">
                  <div class="main_button">
                      <el-button type="primary" size="small" @click="clickTask()"><i class="el-icon-plus"></i>&nbsp;&nbsp;平台发包</el-button>
                  </div>
              </el-col>
          </el-form>
      </el-row>
      <div class="wrap">
          <el-table
              ...
              label="任务状态">
                 <template slot-scope="scope">
              <div>
                <span>{{ scope.row.taskStatus | staskresult }}</span>
              </div>
            </template>
              </el-table-column>
              <el-table-column
        ...
        <el-tabs v-model="activeName" @tab-click="handleClick">
          <el-tab-pane label="基本信息" name="first">
            <el-row>
              ...
              <el-col :span="8"><span class="grid-content bg-purple">任务ID：{{invoiceObj.companyTaskId}}</span></el-col>
              <el-col :span="8"><span class="grid-content bg-purple">任务需求方：{{invoiceObj.companyName}}</span></el-col>
              ...
              <el-col :span="8" v-if="invoiceObj.platformTaskIndustryRes"><span class="grid-content bg-purple">所属行业：
                <span v-for="(item, y) in invoiceObj.platformTaskIndustryRes" :key="y">
                  {{item.industryName}}
                  </span>
                </span></el-col>
              <el-col :span="8"><span class="grid-content bg-purple">需求人数：{{invoiceObj.peopleNumber}}</span></el-col>
              <el-col :span="8"><span class="grid-content bg-purple">任务地点：
                 <span v-for="(item, y) in invoiceObj.platformTaskCityRes" :key="y">
                  {{item.areaName}}
                  </span>
                </span></el-col>
              <el-col :span="8"><span class="grid-content bg-purple">工作时间：{{invoiceObj.createdAt}}至{{invoiceObj.endTime}}</span></el-col>
              <el-col :span="8"><span class="grid-content bg-purple">性别要求：{{invoiceObj.requirementSex | sexresut}}</span></el-col>
              <el-col :span="8"><span class="grid-content bg-purple">学历要求：{{invoiceObj.requirementEducation}}</span></el-col>
              ...
              <el-col :span="8"><span class="grid-content bg-purple">审核状态：{{invoiceObj.auditStatus | invoice}}</span></el-col>
              <el-col :span="24" v-if="invoiceObj.platformTaskLabelRes"><span class="grid-content bg-purple">标签：
                  <span v-for="(item, y) in invoiceObj.platformTaskLabelRes" :key="y">
                  {{item.labelName}}
                  </span>
                </span></el-col>
              <el-col :span="24"><span class="grid-content bg-purple"><span class="grid-left">任务说明：</span>
    ...
    <el-dialog
      ...
      <el-form :model="taskForm" :rules="taskRules" ref="taskForm" label-width="150px" class="demo-ruleForm">
        ...
        <el-form-item label="企业ID：" prop="companyTaskId">
          <el-input v-model="companyTaskId" placeholder="请输入" size="small" @change="changeId"  style="width:500px"></el-input>
        </el-form-item>
...
export default {
  data() {
    return {
      ...
      pageSzie: 10,
      total: null,
      dialogVisibleFa:false,
      dialogVisibleJin:false,
      total: null,
      totaled: null,
       //工作环境
      arr: [],
  ...
  methods: {
      ...
      if (val == 1) {
       ...
       this.dialogVisibleJin = true;
        ...
        this.dialogVisibleFa = true;
      }
    },
    resetFormJin(formName) {
      this.$refs[formName].resetFields();
      this.dialogVisibleJin = false;
    },
    ...
    changeId(val) {
      ...
      this.imgArr = [];
      this.file = [];
      post(api.info4Platform,{
        companyTaskId: val
      }).then(
        (d) => {
          if (d.code === 0) {
            // this.taskForm = d.data;
            this.taskForm.companyTaskId = d.data.id;
            this.taskForm.taskName = d.data.taskName
            this.taskForm.peopleNumber = d.data.peopleNumber
            this.taskForm.requirementSex = d.data.requirementSex
            this.taskForm.requirementAgeMin = d.data.requirementAgeMin
            this.taskForm.requirementAgeMax = d.data.requirementAgeMax
            this.taskForm.endTime = d.data.endTime
            this.taskForm.taskDescribe = d.data.taskDescribe
            this.taskForm.contactMobile = d.data.contactMobile
            this.taskForm.salarySettlement = d.data.salarySettlement
            // this.taskForm['workTimeArr'] = [];
            // this.taskForm['labelArr'] = [];
             console.log(this.taskForm.labelArr,'666666')
            this.taskForm.requirementEducation = d.data.requirementEducationStr
            for (let i = 0; i < d.data.industryList.length; i++) {
              this.hayeData.push(d.data.industryList[i])
            }
            for (let j = 0; j < d.data.cityList.length; j++) {
              this.diquData.push(d.data.cityList[j])
            }
          
            if( d.data.labelList.length > 0){
                for (let k = 0; k < d.data.labelList.length; k++) {
                this.taskForm.labelArr.push(d.data.labelList[k].labelId)
              }
            }

            //  console.log( this.taskForm.labelArr,'77777')
            // console.log(this.taskForm,'666666')
            this.taskForm.workTimeArr.push(d.data.workTimeBegin, d.data.workTimeEnd)
            this.imgArr = d.data.workspace.split(',');
            for (let key = 0; key < this.imgArr.length; key++) {
              let obj = {
                url: this.imgArr[key]
              }
              this.file.push(obj)
    ...
    handleSelect(item) {
      console.log(item);
      this.taskForm.companyTaskId = item.address;
    },
    ...
    submitFormed(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          // console.log(this.hayeData,this.diquData,this.imgArr)
          if (this.hayeData.length == 0 || this.diquData.length == 0) {
          ...
          for (let j = 0; j < this.diquData.length; j++) {
            let obj = {
              areaId: this.diquData[j].areaId || this.diquData[j].taskId
            }
          ...
          this.taskForm['companyTaskId'] = this.companyTaskId;
          ...
          post(api.platformTaskEdit, this.taskForm).then(
            d => {
              if (d.code === 0) {
                this.taskqueryPage();
                this.dialogVisible4 = false
    ...
    taskqueryPage(size) {
      ...
      post(api.taskqueryPage, {
        currentPage: size || 1,
        pageSize: this.pageSzie,
        companyName: this.releaseForm.companyName,
        platformTaskNo: this.releaseForm.platformTaskNo,
        releasedStartTime: this.releaseForm.createDate[0],
        releasedEndTime: this.releaseForm.createDate[1],
        taskStatus:this.releaseForm.taskStatus,
      }).then(
        (d) => {
          loading.close();
          if (d.code === 0) {
    ...
    touPageDataed(size) {
      ...
      post(api.taskApplyPage, {
        currentPage: size || 1,
        pageSize: this.pageSzie,
        companyTaskId: parseInt(this.invoiceObj.companyTaskId),
        platformTaskId: parseInt(this.invoiceObj.id),
      }).then(
        (d) => {
          loading.close();
          if (d.code === 0) {
